...
GetContext::GetContext(
    ...
    const Snapshot* snapshot, PinnableSlice* pinnable_val,
...
GetContext::GetContext(
    const Comparator* ucmp, const MergeOperator* merge_operator, Logger* logger,
    Statistics* statistics, GetState init_state, const Slice& user_key,
    const Snapshot* snapshot, PinnableSlice* pinnable_val, bool* value_found,
    ...
    : GetContext(ucmp, merge_operator, logger, statistics, init_state, user_key,
                 snapshot, pinnable_val, nullptr, value_found, merge_context,
                 do_merge, _max_covering_tombstone_seq, env, seq,
...
bool GetContext::SaveValue(const ParsedInternalKey& parsed_key,
  ...
  if (ucmp_->CompareWithoutTimestamp(parsed_key.user_key, user_key_) == 0) {
    *matched = true;
    ...
    if (ucmp_->timestamp_size() && snapshot_) {
      SequenceNumber seq = snapshot_->GetSequenceNumber();
      if (seq < parsed_key.sequence) {
        return true;  // to continue to the next seq
      }
    }
