...
//                               write finishes
//         GetImpl(ts,seq)
// Since application does not snapshot, it indicates the application does not
// care how RocksDB internally orders this read w.r.t. sequence number.
// Therefore, it is OK to return <k, t1, s1> as long as ts>=t1, even if seq<s1.
...
//                            write finishes
//      scan(ts,seq)
// It is OK for scan to see data whose timestamp is smaller than or equal to
// 100, even if the data may be invisible in terms of sequence number.
// Should application care about sequence number, it should take a snapshot.
// <k, t1, s1> can be seen in scan as long as ts>=t1, even if seq<s1.
